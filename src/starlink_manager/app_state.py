...
class StarlinkManagerState:
    ...
    states = [
        {
            ...
            "timeout": 120,
            "on_timeout": "turn_on",
        },  ## For the moment, only allowed to stay off for a short period
        {"name": "initialising", "timeout": 120, "on_timeout": "no_connection"},
        {
            "name": "not_connected",
            "timeout": 120,
            "on_timeout": "power_off",
        },  ## If not connected after a while, power off, which will start a loop of powering on/off till it works
        {"name": "offline"},
        {"name": "online"},
    ]
    ...
    transitions = [
        ...
        {"trigger": "set_connected", "source": "initialising", "dest": "offline"},
        {
            ...
            "source": ["offline", "online", "initialising"],
            "dest": "not_connected",
        },
